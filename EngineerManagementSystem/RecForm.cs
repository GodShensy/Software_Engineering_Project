...
namespace ManagerClient
{
    public partial class RecForm : UIForm
    {
        ...
        private void uiButton1_Click(object sender, EventArgs e)
        {
            ...
            String sql = "select * from workertab where workerNumber = @pair0;";
            WorkerService service = new WorkerService();
            WorkerSignService service1 = new WorkerSignService();
            String[] parm =
            {
                worker_Number,
            };
            ...
            String sqlDays = "select * from signtab where workerId = @pair0 and signType = 1;";
            this.textBox_workerDay.Text = service1.selelectWorkerInf(sqlDays, parm).Count + "";

            float worker_rec = inflist[0].WorkerBaseRec;
            float mainRec = 0;
            float years = inflist[0].WorkerTime;
            float days = float.Parse(this.textBox_workerDay.Text);
            this.textBox_workerDay.Text = days.ToString();
            float benefits = float.Parse(worker_Benefits);
            float insurance = float.Parse(worker_Insurance);
